...
def checkAllEntries(worksheet,start,end):
    ...
    bad_wb.save("Fuzz_Bad_Cards.xlsx")
    good_wb.save("Fuzz_Good_Cards.xlsx")
    return
...
def getValidMaterials(filename):
    ...
    for line in materialsFile.readlines():
        ...
        validMaterialsArr.append(text)
    return [validMaterials,validMaterialsArr]
...
def fuzzyCheckMaterial(worksheet,start,end):
    file = open("FuzzyMats.txt","w",encoding="utf-8")
    for row in worksheet.iter_rows(min_row=start, max_col=33, max_row=end):
        
        text=row[2].value
        print(text)
        try:
            matches=process.extract(text,validMaterialsArr,limit=5)
        ...
        for match in matches:            
            file.write(str(match[0])+" : "+str(match[1])+"\n")
        file.write("\n")
    file.close()
    return

wb=load_workbook('Old_Bad_Cards.xlsx')
ws =wb['Data Fields']

validMaterials,validMaterialsArr=[getValidMaterials("Valid Materials.txt")[i] for i in (0,1)]
...
fuzzyCheckMaterial(ws,5,ws.max_row)
print('done')